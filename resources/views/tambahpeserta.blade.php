...
<div id="page-title">
    <p><a href="{{url('/welcome')}}">Beranda</a> > <a href="{{url('/event')}}">Event</a> > Tambah Fasilitator dan Peserta</p>
    <br>
    <h3>Tambah Fasilitator Pelatihan</h3>
</div>
...
<!-- @foreach($loop_fas as $l)
    @for($i=1;$i<=$l->JmlFas;$i++)
    <select name="fasilitator[]" class="form-control pull-right">
    <OPTION>{{'Fasilitator '. $i}}</OPTION>
    @foreach($fasilitator as $p)
...
@endforeach
 -->
<select name="fasilitator1" id="fasilitator1" class="form-control pull-right">
    <option>Fasilitator 1</option>
    @foreach($fasilitator as $p)
        <option value="{{$p->NIK}}">{{$p->NIK.' '.$p->Nama}}</option>
    @endforeach
</select>
<select name="fasilitator2" id="fasilitator2" class='form-control pull-right'>
    <option>Fasilitator 2</option>
    @foreach($fasilitator as $p)
        <option value="{{$p->NIK}}">{{$p->NIK.' '.$p->Nama}}</option>
    @endforeach
</select>
...
<!-- @foreach($loop_pes as $l)
    @for($i=1;$i<=$l->JmlPeserta;$i++)
    <select name="peserta[{{$i}}]" class="form-control pull-right">
    <OPTION>{{'Peserta '. $i}}</OPTION>
    ...
    @endfor
@endforeach -->
<select name='peserta1' id='peserta1' class="form-control pull-right">
    <option>Peserta 1</option>
    @foreach($peserta as $p)
        <option value="{{$p->NIK}}">{{$p->NIK.' '.$p->Nama}}</option>
    @endforeach
</select>
<select name='peserta2' id='peserta2' class="form-control pull-right">
    <option>Peserta 2</option>
    @foreach($peserta as $p)
        <option value="{{$p->NIK}}">{{$p->NIK.' '.$p->Nama}}</option>
    @endforeach
</select>
<select name='peserta3' id='peserta3' class="form-control pull-right">
    <option>Peserta 3</option>
    @foreach($peserta as $p)
        <option value="{{$p->NIK}}">{{$p->NIK.' '.$p->Nama}}</option>
    @endforeach
</select>
<select name='peserta4' id='peserta4' class="form-control pull-right">
    <option>Peserta 4</option>
    @foreach($peserta as $p)
        <option value="{{$p->NIK}}">{{$p->NIK.' '.$p->Nama}}</option>
    @endforeach
</select>
...
@foreach($event as $e)
<input type="hidden" name="tglMulai" id="tglMulai" value="{{$e->TglMulaiEvent}}">
@endforeach
</div>
</div>
</div>
...
<div class="form-group">
             <div class="input-group">
              ...
              <button class="btn btn-primary center-block" onclick="tes();" name="btn_date1" id="btn_date1">Simpan</button>
...
<script type="text/javascript">
    function tes(){
        var fas1 = $("#fasilitator1").val();
        var fas2 = $("#fasilitator2").val();
        var pes1 = $("#peserta1").val();
        var pes2 = $("#peserta2").val();
        var pes3 = $("#peserta3").val();
        var pes4 = $("#peserta4").val();
        var ids = $("#ids").val();
        var tglMulai = $("#tglMulai").val();
        $.ajax({
            type: "GET",
            url: "{{url('tambahPesertaDone/api')}}",
            data: {'fas1':fas1,
                'fas2':fas2,
                'pes1':pes1,
                'pes2':pes2,
